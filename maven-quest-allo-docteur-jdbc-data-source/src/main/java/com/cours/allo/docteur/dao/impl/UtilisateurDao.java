...
public class UtilisateurDao extends AbstractDao<Utilisateur> implements IUtilisateurDao {
    ...
    private final static String SQL_SELECT_CODE_POSTALE = "SELECT * FROM utilisateur LEFT JOIN adresse ON utilisateur.idUtilisateur = adresse.idUtilisateur "
            + "WHERE adresse.codePostal=?";
    private final static String SQL_INSERT = "INSERT INTO utilisateur(idUtilisateur,civilite,prenom,nom,identifiant,motPasse,dateNaissance,"
            + "dateCreation,dateModification,actif,marquerEffacer,version) "
            + "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1);";
